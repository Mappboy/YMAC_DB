...
def get_claim_group(claim_group, survey_code):
    ...
    :param claim_group:
    :return:
    >>> survey_group = get_claim_group("Yinhawangka (For Heritage)", 'YHW018-132')
...
def find_survey_folder(survey_code, orig_survey, created_on, claim_group):
    ...
    claim_folder = os.path.join(os.path.normpath(r"Z:\Claim Groups"),"{}\\Heritage Surveys\\{}".format(claim_name,created_on.year))
...
def get_survey_status(raw_status):
    """
    Return survey cleaned survey status
    :param raw_status:
    :return:
    """
    return None
...
if __name__ == "__main__":
    ...
    with open(crm_file,"r") as crm_csv:
        ...
        for row in reader:
            ...
            survey_status = get_survey_status(row["Survey Status"])
            survey_type = get_survey_type(row["Survey Type 1"])
            print(survey_code, trip_number, created_by, survey_group, folder_location, proponent, methodologies, survey_status, survey_type, original_ymac_code)
    doctest.testmod()
